  ...
  useEffect(() => {
    getCourses()
      .then(d => {
        ...
        setCourses(list);
        if (list.length) setCourseId(list[0].id); // default to whatever's actually available
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, []);

  const canJoin = name.trim() && courseId;
      ...
      <div style={{ textAlign:"center", width:"90%", maxWidth:400, padding:20 }}>
        ...
        <div style={{ background:"#1A1640", borderRadius:16, padding:24, border:"1px solid #2D2757" }}>
          ...
          <input value={name} onChange={e=>setName(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&canJoin&&onJoin(name.trim(),courseId)}
          ...
          <select value={courseId} onChange={e=>setCourseId(e.target.value)}
            ...
            {!loading && courses.length === 0 && <option value="">No courses yet — ask a lecturer to add one</option>}
            {courses.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
          </select>

          <button onClick={()=>canJoin && onJoin(name.trim(),courseId)} disabled={!canJoin}
